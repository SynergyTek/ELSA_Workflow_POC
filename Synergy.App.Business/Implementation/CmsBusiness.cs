    ...
    : BusinessBase<TemplateViewModel, TemplateModel>(repo, serviceProvider), ICmsBusiness
{
    ...
    public async Task ManageTable(TableViewModel table)
    {
        var existingTableMetadata =
            await _repo.GetSingleById<TableViewModel, TableModel>(table.Id);
        if (existingTableMetadata != null)
        {
            existingTableMetadata.OldName = table.OldName;
            existingTableMetadata.OldSchema = table.OldSchema;
    ...
    private async Task EditTable(TableViewModel table)
    {
        var existColumn = await cmsQueryBusiness.GetEditFormTableExistColumn(table);
    ...
    private async Task CreateTable(TableViewModel table, bool dropTable)
    {
        ...
        foreach (var column in columns)
        {
            ...
            if (column is { IsForeignKey: true, IsVirtualForeignKey: false, IsMultiValueColumn: false })
            {
                fk +=
