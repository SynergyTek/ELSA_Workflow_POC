        ...
        : BusinessBase<TemplateViewModel, Data.Models.Template>(repo, serviceProvider), ICmsBusiness
    {
        ...
        public async Task ManageTable(TableMetadataViewModel tableMetadata)
        {
            ...
            if (existingTableMetadata != null)
            {
                ...
                var tableExists = await cmsQueryBusiness.ManageTableExists(existingTableMetadata);
                if (tableExists)
                {
                    var recordExists = await cmsQueryBusiness.ManageTableRecordExists(existingTableMetadata);
        ...
        {
            var existColumn = await cmsQueryBusiness.GetEditFormTableExistColumn(tableMetadata);


            var constraints = await cmsQueryBusiness.GetEditFormTableConstraints(tableMetadata);
            ...
            foreach (var existingMetadaColumn in existingMetadaColumnList)
            {
                ...
                if (isInTable != null)
                {
                    if (postgreType != isInTable.DataTypestr)
                    {
                        alterColumnScriptList.Add(
                            $"{Environment.NewLine}ALTER COLUMN \"{existingMetadaColumn.Name}\" TYPE {postgreType}");
                    ...
                    else if (existingMetadaColumn.IsNullable != isInTable.IsNullable)
                    {
                        ...
                        {
                            alterColumnScriptList.Add(
                                $"{Environment.NewLine}ALTER COLUMN \"{existingMetadaColumn.Name}\"  DROP NOT NULL");
                        }
                        else
                        {
                            alterColumnScriptList.Add(
                                $"{Environment.NewLine}ALTER COLUMN \"{existingMetadaColumn.Name}\"  SET NOT NULL");
                        }
                    }
                }
                else
                {
                    ...
                    {
                        alterColumnScriptList.Add(
                            $"{Environment.NewLine}ADD COLUMN \"{existingMetadaColumn.Name}\" {postgreType} {nullSet} COLLATE {ApplicationConstant.Database.Schema._Public}.{ApplicationConstant.Database.DefaultCollation}");
                    }
                    else
                    {
                        alterColumnScriptList.Add(
                            $"{Environment.NewLine}ADD COLUMN \"{existingMetadaColumn.Name}\" {postgreType} {nullSet}");
                    }
        ...
        private async Task CreateTable(TableMetadataViewModel tableMetadata, bool dropTable)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    fk +=
                        $@"CONSTRAINT ""{column.ForeignKeyConstraintName}"" FOREIGN KEY (""{column.Name}"") REFERENCES {column.ForeignKeyTableSchemaName}.""{column.ForeignKeyTableName}"" (""{column.ForeignKeyColumnName}"") MATCH SIMPLE ON UPDATE NO ACTION  ON DELETE SET NULL,";
                }

                columnText = $"\"{column.Name}\" {type}";
                if (column.DataType == Data.DataColumnTypeEnum.Text)
                {
                    columnText = string.Concat(columnText,
                        $" COLLATE {ApplicationConstant.Database.Schema._Public}.{ApplicationConstant.Database.DefaultCollation}");
                }
            ...
            await cmsQueryBusiness.TableQueryExecute(tableQuery);
        }
        ...
        // public async Task<DataTable> GetDataListByTemplate(string templateCode, Guid templateId, string? where = null)
        // {
        //     var template =
        //         await _repo.GetSingle<TemplateViewModel, Template>(x => x.Code == templateCode || x.Id == templateId);
        //
        //     var tableMetadata =
        //         await _repo.GetSingleById<TableMetadataViewModel, TableMetadata>(template.UdfTableMetadataId);
        //
