...
namespace Synergy.App.Business.Implementation
{
            ...
            IColumnMetadataBusiness
    {
        ...
        private async Task<List<ColumnMetadataViewModel>> GetViewableForeignKeyColumnListForNote(Guid tableMetadataId,
            List<ColumnMetadataViewModel> columnList)
        {
            var list = new List<ColumnMetadataViewModel>();
            var fks = columnList.Where(x => x.IsForeignKey && x.ForeignKeyTableId != Guid.Empty);
            if (fks != null && fks.Any())
            {
                var result = await cmsQueryBusiness.GetViewableForeignKeyColumnListForNoteData(tableMetadataId);
                if (result != null && result.Any())
                {
                    foreach (var item in result)
                    {
        ...
        private async Task<List<ColumnMetadataViewModel>> GetViewableForeignKeyColumnListForTask(Guid tableMetadataId,
            List<ColumnMetadataViewModel> columnList)
        {
            var list = new List<ColumnMetadataViewModel>();
            var fks = columnList.Where(x => x.IsForeignKey && x.ForeignKeyTableId != Guid.Empty);
            if (fks != null && fks.Any())
            {
                var result = await cmsQueryBusiness.GetViewableForeignKeyColumnListForTaskData(tableMetadataId);
                if (result != null && result.Any())
                {
                    foreach (var item in result)
                    {
            ...
            var result1 = await cmsQueryBusiness.GetViewableForeignKeyColumnListForTaskData2();
            list.AddRange(result1);
            return list;
        }

        private async Task<List<ColumnMetadataViewModel>> GetViewableForeignKeyColumnListForService(
            Guid tableMetadataId, List<ColumnMetadataViewModel> columnList)
        {
            var list = new List<ColumnMetadataViewModel>();
            var fks = columnList.Where(x => x.IsForeignKey && x.ForeignKeyTableId != Guid.Empty);
            if (fks != null && fks.Any())
            {
                var result = await cmsQueryBusiness.GetViewableForeignKeyColumnListForServiceData(tableMetadataId);
                if (result != null && result.Any())
                {
                    foreach (var item in result)
                    {
            ...
            var result1 = await cmsQueryBusiness.GetViewableForeignKeyColumnListForServiceData1(tableMetadataId);
            list.AddRange(result1);
            return list;
        }
    }
