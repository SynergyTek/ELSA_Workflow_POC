...
public class Helper
{
     public static Dictionary<string, string> QueryStringToDictionary(string value)
        ...
        // }
        public static string TransformWorkspaceName(string workspaceName)
        {
            if (string.IsNullOrEmpty(workspaceName))
                return string.Empty;

            // Split the workspace name by spaces
            string[] parts = workspaceName.Split(' ');

            // Transform each part: replace spaces with underscores, then convert to uppercase
            for (int i = 0; i < parts.Length; i++)
            {
                parts[i] = parts[i].Replace(" ", "_").ToUpper();
            }
